...
export function WeekStrip({ sessions }) {
  ...
  return (
    <View style={s.wrap}>
      <View style={s.row}>
        <Text style={text.eyebrowSmall}>THIS WEEK</Text>
        <Text style={s.count}>{total} workout{total === 1 ? '' : 's'}</Text>
      </View>
      <View style={s.bars}>
        {days.map((d, i) => {
...
const s = StyleSheet.create({
  ...
  count: { ...text.bodySecondary, fontSize: 12, fontWeight: '600' },
  ...
  todayDot: { borderWidth: 1.5, borderColor: colors.text, borderRadius: 4, height: '20%', backgroundColor: 'transparent' },
