...
import { EXERCISE_REST_SECONDS } from '../constants/workoutConstants';
import type { DayTemplate, WorkoutConfig } from '../types/workoutTypes';

export type UseWorkoutConfig = {
  config: WorkoutConfig;
  loaded: boolean;
  updateDay: (dayIndex: number, updates: Partial<DayTemplate>) => void;
  addDay: () => void;
...
export function useWorkoutConfig(): UseWorkoutConfig {
  const [config, setConfig, loaded] = usePersistedState<WorkoutConfig>(
    KEYS.config,
    defaultConfig,
  );

  const updateDay = useCallback(
    (dayIndex: number, updates: Partial<DayTemplate>) => {
      setConfig(prev => ({
        ...prev,
  ...
  const addDay = useCallback(() => {
    setConfig(prev => {
      ...
      const color =
        dayPalette.find(c => !used.has(c)) ?? dayPalette[prev.days.length % dayPalette.length];
      return {
        ...prev,
        days: [
          ...prev.days,
