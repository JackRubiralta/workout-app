...
export type DayEditSheetProps = {
  ...
  dayIndex: number;
  daysCount: number;
  onSave: (dayIndex: number, updates: Partial<DayTemplate>) => void;
...
export function DayEditSheet({
  ...
  const handleSaveDay = () => {
    ...
    onSave(dayIndex, {
      ...
      focus: focus.trim(),
      exerciseRestSeconds:
        !isNaN(rest) && rest >= 30 ? Math.min(rest, 600) : day.exerciseRestSeconds ?? 180,
      exercises,
    });
    onClose();
  };

  const handleSaveExercise = (exIndex: number, updates: Partial<ExerciseTemplate>) => {
    setExercises(prev => prev.map((ex, i) => (i === exIndex ? { ...ex, ...updates } : ex)));
    setEditingExIndex(null);
  };

  const handleDeleteExercise = () => {
    if (exercises.length <= 1 || editingExIndex == null) return;
    ...
    <>
      <Sheet visible={visible && editingExIndex === null} onClose={onClose} flex height="92%">
        <SheetHeader
          left={
            <View
              style={[s.dayDot, { backgroundColor: day.color + '20', borderColor: day.color + '40' }]}
            >
                  ...
                  <TouchableOpacity style={s.exRow} onPress={() => setEditingExIndex(i)} activeOpacity={0.7}>
                    ...
                    <View style={s.exInfo}>
                      ...
                      <Text style={s.exMeta} numberOfLines={1}>
                        {warmupLabel}
                        {ex.sets} sets · {ex.reps} · {ex.restSeconds}s rest{tag ? ` · ${tag}` : ''}
                      </Text>
                    </View>
...
const s = StyleSheet.create({
  ...
  unit: { ...(text.monoSubhead as TextStyle), fontWeight: '600', width: 28 },
  hint: { ...(text.monoCaption as TextStyle), marginTop: spacing.xs, letterSpacing: 0.3 },
  ...
  exBadgeText: { ...(text.eyebrowSmall as TextStyle), fontSize: fontSize.footnote, fontWeight: '700' },
  exInfo: { flex: 1, gap: 2 },
  exName: { ...(text.monoSubhead as TextStyle), color: colors.text, fontWeight: '600' },
  exMeta: { ...(text.monoCaption as TextStyle), color: colors.textSecondary, letterSpacing: 0.2 },
  exReorder: { gap: 2 },
  ...
  addRowText: { ...(text.monoSubhead as TextStyle), fontWeight: '600' },
  ...
  deleteBtnText: { ...(text.monoSubhead as TextStyle), fontWeight: '600', color: colors.danger, letterSpacing: 0.3 },
});
