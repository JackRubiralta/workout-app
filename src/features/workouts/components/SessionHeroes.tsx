...
import { colors, fontSize, radius, spacing, fonts, text } from '@/shared/theme';
import { CircularTimer } from './CircularTimer';
import { getRepsGuide, getSetLabel } from '../constants/exerciseDefaults';
import type { DayTemplate, SetPosition } from '../types/workoutTypes';

export type ExerciseHeroProps = {
  day: DayTemplate;
  exIndex: number;
  setIndex: number;
  nameFontSize: number;
...
export function ExerciseHero({ day, exIndex, setIndex, nameFontSize, isSmall }: ExerciseHeroProps) {
...
export type RestHeroProps = {
  secondsLeft: number;
  totalSeconds: number;
  nextPos: SetPosition | null;
  day: DayTemplate;
  timerSize: number;
  isSmall: boolean;
};

export function RestHero({ secondsLeft, totalSeconds, nextPos, day, timerSize, isSmall }: RestHeroProps) {
  const nextEx = nextPos ? day.exercises[nextPos.e] : null;
  const nextLabel = nextEx && nextPos ? getSetLabel(nextEx, nextPos.s) : null;
  return (
      ...
      <View style={s.nextUp}>
        {nextEx ? (
          <>
            ...
            <Text style={s.upNextEx} numberOfLines={1}>
              {nextEx.name}
            </Text>
...
export function CompletionHero({ day, doneSets }: CompletionHeroProps) {
  return (
...
const s = StyleSheet.create({
  ...
  counter: { ...(text.eyebrow as TextStyle), color: colors.textSecondary, letterSpacing: 1 },
  label: { ...(text.setLabel as TextStyle) },
  name: { ...(text.exerciseName as TextStyle) },
  ...
  upNextLabel: { ...(text.eyebrow as TextStyle), color: colors.textTertiary, letterSpacing: 1.2 },
  upNextEx: { ...(text.title3 as TextStyle), fontWeight: '600', textAlign: 'center' },
  upNextSet: { ...(text.monoSubhead as TextStyle), fontWeight: '600', letterSpacing: 0.3 },
  ...
  completionTitle: { ...(text.title2 as TextStyle) },
  completionSubtitle: { ...(text.monoSubhead as TextStyle), fontSize: fontSize.callout },
  completionStats: { ...(text.monoSubhead as TextStyle), color: colors.textTertiary },
});
