...
import { Sheet, SectionLabel, Sparkline, StatCard } from '@/shared/components';
import { useSessionData, useSettingsData } from '@/shared/state/store';
import { topSetPerSession, personalRecords, epley } from '../utils/suggestionsUtils';
import { formatDateShort } from '@/shared/utils/date';
import { fromLb, unitLabel } from '@/shared/utils/units';
import { MAX_HISTORY_POINTS } from '@/features/tracking/constants/trackingConstants';
...
export function ExerciseHistorySheet({
  ...
  const topSets = useMemo(() => {
    ...
    return topSetPerSession(sessions, exerciseName)
  ...
  const e1rmPoints = useMemo(
    () =>
      topSets.map(({ entry }) => ({
        value: Math.round(fromLb(epley(entry.weight, entry.reps), unitSystem)),
      })),
    [topSets, unitSystem],
  );
    ...
    <Sheet visible={visible} onClose={onClose}>
      <View style={s.body}>
        <View style={s.header}>
          ...
          <Text
            style={[text.title3 as TextStyle, { fontSize: fontSize.headline, flex: 1 }]}
            numberOfLines={1}
          >
            {exerciseName ?? ''}
          </Text>
            ...
            <>
              <View style={s.statsGrid}>
                ...
                <StatCard
                  value={
                    prs.bestE1RM
                      ? Math.round(fromLb(epley(prs.bestE1RM.weight, prs.bestE1RM.reps), unitSystem))
                      : '—'
                  }
                  label="e1RM (Epley)"
                  sub={
                    prs.bestE1RM
                      ? `${round1(fromLb(prs.bestE1RM.weight, unitSystem))} × ${prs.bestE1RM.reps}`
                      : undefined
                  }
                />
              </View>
              ...
              <View style={s.chartSection}>
                <SectionLabel style={s.chartLabel}>
                  TOP SET ({unit}) · LAST {topSets.length}
                </SectionLabel>
              ...
              <View style={s.list}>
                ...
                {[...topSets].reverse().map(({ entry, session }, i) => (
                  <View key={i} style={s.row}>
                    ...
                    <Text style={s.rowDay} numberOfLines={1}>
                      {session.dayTitle}
                    </Text>
                    <Text style={s.rowValue}>
                      {round1(fromLb(entry.weight, unitSystem))}
                      <Text style={s.rowUnit}> {unit}</Text> × {entry.reps}
                    </Text>
                  </View>
                ))}
              </View>
...
const s = StyleSheet.create({
  ...
  emptyTitle: { ...(text.title3 as TextStyle), color: colors.textSecondary },
  emptySub: {
  ...
  rowDate: { ...(text.monoCaption as TextStyle), color: colors.textSecondary, width: 56 },
  rowDay: { flex: 1, ...(text.monoFootnote as TextStyle), color: colors.text, fontWeight: '600' },
  rowValue: { ...(text.monoSubhead as TextStyle), fontWeight: '700' },
