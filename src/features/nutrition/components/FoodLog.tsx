...
import { colors, fonts, radius, rowPadding, spacing, surfaces, text } from '@/shared/theme';
import { SectionLabel } from '@/shared/components/SectionLabel';
import { CameraIcon } from '@/shared/components/icons';
import { formatTime } from '@/shared/utils/date';
import { copy } from '@/shared/copy';
import { formatFoodMeta } from '../utils/nutritionMath';
import type { FoodEntry } from '../types/nutritionTypes';
...
function FoodPill({ item, onPress }: FoodPillProps) {
...
export function FoodLog({ items, onPressItem }: FoodLogProps) {
  return (
    <View style={s.wrap}>
...
const s = StyleSheet.create({
  ...
  timeCol: { width: 56 },
  time: { ...(text.monoCaption as TextStyle), color: colors.textSecondary, fontWeight: '700', letterSpacing: 0.3 },
  main: { flex: 1, gap: 2 },
  nameRow: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  name: { ...(text.title3 as TextStyle), fontSize: 15, color: colors.text, fontWeight: '700', flexShrink: 1 },
  photoBadge: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 2,
  ...
  emptyTitle: { ...(text.title3 as TextStyle), fontSize: 17, color: colors.text },
  emptySub: { ...(text.bodySecondary as TextStyle), textAlign: 'center', lineHeight: 20, fontSize: 14 },
});
