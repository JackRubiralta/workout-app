...
import { colors, macroColors, spacing, surfaces, text } from '@/shared/theme';
import { BarChart, SectionLabel, TrendChip } from '@/shared/components';
import { totalsForDay } from '../utils/nutritionMath';
import { formatDateKey } from '@/shared/utils/date';
import type { LogsByDate, MacroGoals } from '../types/nutritionTypes';
...
type SeriesPoint = { d: Date; calories: number; protein: number; isToday: boolean };

function buildSeries(logsByDate: LogsByDate, days = DAYS): SeriesPoint[] {
...
function avgVs(prev: number, recent: number): number | null {
  if (!prev || prev <= 0) return null;
  return Math.round(((recent - prev) / prev) * 100);
}

export type NutritionTrendsProps = {
  logsByDate: LogsByDate;
  goals: MacroGoals;
};

export function NutritionTrends({ logsByDate, goals }: NutritionTrendsProps) {
  const series = buildSeries(logsByDate);
  const prevSeries = (() => {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const out: { calories: number; protein: number }[] = [];
    for (let i = 13; i >= 7; i--) {
      const d = new Date(today);
      d.setDate(today.getDate() - i);
      const items = logsByDate[formatDateKey(d)] ?? [];
        ...
        <View style={s.cardHeader}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={s.cardSub}>
              avg <Text style={s.cardSubVal}>{avg7Cal}</Text> · goal {goals.calories}
            </Text>
          </View>
      ...
      <View style={s.card}>
        <View style={s.cardHeader}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={s.cardSub}>
              avg <Text style={s.cardSubVal}>{avg7Pro}g</Text> · goal {goals.protein}g
            </Text>
          </View>
