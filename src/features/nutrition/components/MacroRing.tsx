...
function geometry(size: number, strokeWidth: number) {
  const r = (size - strokeWidth) / 2;
  return { r, c: 2 * Math.PI * r, cx: size / 2 };
}

function useMountedValue(target: number, duration = 750): number {
...
export function CalorieRing({ value = 0, goal = 2000, color = colors.text, size = 168 }: CalorieRingProps) {
  const STROKE = 12;
...
export function MacroRing({ label, value = 0, goal = 0, unit = 'g', color, size = 70 }: MacroRingProps) {
  const STROKE = 6;
  const { r, c, cx } = geometry(size, STROKE);
  const targetPct = goal > 0 ? Math.min(value / goal, 1.5) : 0;
  const animated = useMountedValue(targetPct);
