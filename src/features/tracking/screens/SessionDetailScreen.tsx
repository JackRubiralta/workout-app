...
import { useSessionData, useSettingsData } from '@/shared/state/store';
import { DetailHeader, IconButton, StatCard, StatusPill } from '@/shared/components';
import { TrashIcon } from '@/shared/components/icons';
import { ExerciseHistorySheet } from '@/features/workouts/components/ExerciseHistorySheet';
import { sessionVolume } from '@/features/workouts/utils/volumeUtils';
import { formatDurationISO } from '@/shared/utils/format';
import { formatWeight, fromLb, unitLabel } from '@/shared/utils/units';
import { formatDateHeading } from '@/shared/utils/date';
import { confirm } from '@/shared/utils/confirm';
import { copy } from '@/shared/copy';
import type { SetEntry } from '@/features/workouts/types/workoutTypes';

type ExerciseGroup = { name: string; sets: SetEntry[] };

function groupByExercise(entries: ReadonlyArray<SetEntry>): ExerciseGroup[] {
  const out: ExerciseGroup[] = [];
  const map = new Map<string, ExerciseGroup>();
  for (const entry of entries) {
    if (!map.has(entry.exerciseName)) {
      const g: ExerciseGroup = { name: entry.exerciseName, sets: [] };
...
export function SessionDetailScreen() {
  ...
  const volumeDisplay = useMemo(
    () => Math.round(fromLb(volumeLb, unitSystem)),
    [volumeLb, unitSystem],
  );
    ...
    <SafeAreaView style={s.container} edges={['top']}>
      <DetailHeader
        ...
        center={
          <View
            style={[
              s.dayDot,
              { backgroundColor: session.dayColor + '20', borderColor: session.dayColor + '40' },
            ]}
          />
        }
        right={
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        <View style={s.titleArea}>
          <Text style={[text.largeTitle as TextStyle, { color: session.dayColor }]}>
            {session.dayTitle}
          </Text>
        ...
        <View style={s.statsRow}>
          ...
          <StatCard
            value={session.entries.filter(e => !e.isPlaceholder).length}
            label="SETS"
          />
          <StatCard
          ...
          />
          <StatCard
            value={formatDurationISO(session.startedAt, session.completedAt) ?? '—'}
            label="DURATION"
          />
        </View>
                      ...
                      >
                        ...
                        <View style={s.setData}>
                          {hasTime && !hasWeight && !hasReps ? (
                            <Text style={s.weight}>
                              {Math.floor((entry.timeSeconds ?? 0) / 60)}:
                              {String((entry.timeSeconds ?? 0) % 60).padStart(2, '0')}
                            ...
                            <>
                              ...
                              {entry.toFailure && (
                                <View
                                  style={[s.failBadge, { backgroundColor: session.dayColor + '20' }]}
                                >
...
const s = StyleSheet.create({
  ...
  date: { ...(text.monoCaption as TextStyle), color: colors.textTertiary, marginTop: 4 },
  ...
  emptyText: { ...(text.monoFootnote as TextStyle), color: colors.textTertiary },
  ...
  exHint: { ...(text.monoCaption as TextStyle), fontWeight: '600', letterSpacing: 0.5 },
  ...
  setLabel: {
    ...(text.monoFootnote as TextStyle),
  ...
  times: { ...(text.monoFootnote as TextStyle), color: colors.textTertiary },
  reps: { ...(text.monoSubhead as TextStyle), fontWeight: '700', color: colors.text, minWidth: 20 },
  failBadge: {
    width: 18,
    height: 18,
