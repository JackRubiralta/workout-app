...
export const shadow = {
  sm: Platform.select({
    ...
    android: { elevation: 2 },
    default: { boxShadow: '0 2px 8px rgba(0,0,0,0.35)' },
  }),
  md: Platform.select({
    ios: { shadowColor: '#000', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.4, shadowRadius: 16 },
    android: { elevation: 6 },
    default: { boxShadow: '0 6px 16px rgba(0,0,0,0.4)' },
  }),
  lg: Platform.select({
    ios: { shadowColor: '#000', shadowOffset: { width: 0, height: 12 }, shadowOpacity: 0.5, shadowRadius: 24 },
    android: { elevation: 12 },
    default: { boxShadow: '0 12px 24px rgba(0,0,0,0.5)' },
  }),
  // Coloured glow — pass a hex color, returns a shadow that matches.
  glow: (hex, intensity = 0.55) => Platform.select({
    ios: { shadowColor: hex, shadowOffset: { width: 0, height: 6 }, shadowOpacity: intensity, shadowRadius: 18 },
    android: { elevation: 10 },
    default: { boxShadow: `0 6px 20px ${hex}55` },
  }),
};
